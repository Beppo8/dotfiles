...
keys.globalkeys = gears.table.join(awful.key({modkey}, "s",
    ...
    end), awful.key({altkey, "Shift"}, "Tab", function()
        apps.switcher.switch(-1, "Mod1", "Alt_L", "Shift", "Tab")
    end),
    ...
    awful.key({modkey}, "i", function () awesome.emit_signal("dashboard::toggle") end,
        {description = "open the menu app", group = "launcher"}),
    ...
    -- useless gaps
    awful.key({modkey}, "g",
        function() beautiful.useless_gap = beautiful.useless_gap + 5 end,
    ...
    end, {description = "restore minimized", group = "client"}), awful.key(
        {modkey}, "r", function() awful.spawn(apps.launcher, false) end),
    awful.key({modkey}, "p", function()
        awful.spawn.easy_async_with_shell("escrotum -C", function(stdout)
            local fileName = string.gsub(stdout, "\n", "")
            require("widgets.notifications").screenshot(fileName)
        end)
    end), 
    awful.key({modkey, "Shift"}, "p", function()
        awful.spawn.easy_async_with_shell("escrotum -s -C", function(stdout)
            local fileName = string.gsub(stdout, "\n", "")
            require("widgets.notifications").screenshot(fileName)
        end)
    end), 
    ...
    awful.key({modkey}, "space", function()
        local handle = io.popen("xkblayout-state print '%n'")
        local current_layout = handle:read("*a")
        handle:close()

        if current_layout == "English" then
            awful.spawn("setxkbmap es", false) 
        elseif current_layout == "Spanish" then 
            awful.spawn("setxkbmap us", false)
    ...
    awful.key({}, "XF86MonBrightnessDown", function()
        awful.spawn.easy_async_with_shell("brightnessctl set 3%- > /dev/null",
            function(stdout)
...
keys.clientkeys = gears.table.join(awful.key({modkey}, "f", function(c)
